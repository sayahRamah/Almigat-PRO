...
# 🆕 أمر المالك: إحصائيات المشتركين
async def show_subscribers_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != OWNER_ID:
        await update.message.reply_text("❌ هذا الأمر مخصص للمالك فقط.", parse_mode='HTML')
        return

    try:
        total_users, premium_users = get_user_counts()
        
        report = f"📊 <b>إحصائيات المشتركين</b>\n\n"
        report += f"👤 <b>إجمالي المستخدمين المسجلين:</b> {total_users}\n"
        report += f"⭐️ <b>المشتركين المميزين (Active Premium):</b> {premium_users}"
        
        await update.message.reply_text(report, parse_mode='HTML')
        
    except Exception as e:
        logging.error(f"فشل إرسال إحصائيات المشتركين: {e}")
        await update.message.reply_text("❌ عذراً، حدث خطأ أثناء جلب الإحصائيات.", parse_mode='HTML')

# 🆕 أمر المالك: فحص حالة الجدولة
async def check_jobs_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not scheduler:
        await update.message.reply_text("❌ لم يتم تشغيل مُجدول المهام بعد.", parse_mode='HTML')
        return

    # 1. فحص حالة API
    api_status_ok, api_message = get_prayer_api_status()
    
    report = f"🛠️ <b>تقرير فحص حالة البوت والجدولة</b>\n"
    report += "--------------------------------------\n"
    report += f"🌐 <b>حالة API مواقيت الصلاة:</b>\n{api_message}\n"
    report += "--------------------------------------\n"
    ...
    report += f"⏱️ <b>المهام المُجدوَلة حالياً:</b>\n"
    report += f"  - صلوات مجدولة (فردية): <b>{prayer_jobs_count}</b>\n"
    report += f"  - أذكار المساء مجدولة: <b>{azkar_masaa_jobs_count}</b>\n"
    report += f"  - مهام CRON يومية (جدولة/انتهاء): <b>{cron_jobs_count}</b>\n"
    report += f"  - إجمالي المهام في قائمة الانتظار: <b>{len(jobs)}</b>\n"
    
    # فحص موعد تشغيل CRON التالي (لتحديث الجدولة)
    try:
        ...
        if daily_schedule_job:
            ...
            report += f"\n🗓️ <b>موعد الجدولة اليومية القادم (01:00):</b>\n"
            report += f"  - {next_run}"
...
def main():
    ...
    application.add_handler(CommandHandler("stats", show_subscribers_command)) # 🆕 الأمر الجديد
